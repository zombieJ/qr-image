...
function getIntensity(matrix) {
    ...
    for (var x = 1; x < N - 1; x++) {
        for (var y = 1; y < N - 1; y++) {
            ...
            if(matrix[y-1][x-1]) count += matrix[y][x-1] || matrix[y-1][x] ? 1 : 0.1;
            if(matrix[y-1][x+1]) count += matrix[y][x+1] || matrix[y-1][x] ? 1 : 0.1;
            if(matrix[y+1][x-1]) count += matrix[y][x-1] || matrix[y+1][x] ? 1 : 0.1;
            if(matrix[y+1][x+1]) count += matrix[y][x+1] || matrix[y+1][x] ? 1 : 0.1;
    ...
    return Math.floor(intensity * 100) / 100;
}

// {{{1 All-in-one function
function getMatrix(data, preferMask) {
    ...
    for (var mask = 0; mask < 8; mask++) {
        ...
        if(preferMask === mask || preferMask === undefined || preferMask === null) {
            var pc = pointCount(matrix);
            var it = getIntensity(matrix);
            console.log(
                "Mask:", mask,
                "/ Mask Penalty:", p,
                "/ Point Count:", pc,
                "/ Intensity:", it,
                "/ Balance:", Math.floor(p * it / pc * 100) / 100,
                "/ Weight:", Math.floor(p / pc * 100) / 100
            );
        }
    }

    if(preferMask === undefined || preferMask === null) {
        console.log("Best Mask:", bestMask);
    }

    fillData(matrix, data, preferMask || bestMask);
    fillReserved(matrix, data.ec_level, preferMask || bestMask);
